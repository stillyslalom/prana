function prana_fig = prana_LayoutFcn(policy,gui_selfcal)
...
if gui_selfcal
    gui_name = 'selfcalprana';
    persistent hsingleton_selfcal;
    if strcmpi(policy, 'reuse') & ishandle(hsingleton_selfcal)
        prana_fig = hsingleton_selfcal;
        return;
    end
else
    gui_name = 'prana';
...
create_imagepanel_controls(prana_fig,gui_selfcal)

% --- create particlepanel with controls for PTV and sizing
create_particlepanel_controls(prana_fig,gui_selfcal)

% --- create exppanel with controls for defining the physical exp. params
create_exppanel_controls(prana_fig,gui_selfcal)
...
% top of them
create_panel_toggles(prana_fig,gui_selfcal)

if gui_selfcal
    hsingleton_selfcal = prana_fig;
else
...
% --- create pushbutton toggles to switch between the different panels
function create_panel_toggles(prana_fig,gui_selfcal)
...
% --- create imagepanel with controls for Image and Data I/O
function create_imagepanel_controls(prana_fig,gui_selfcal)

if gui_selfcal
    gui_selfcalOpt = 'off';
    gui_imagepanelVis = 'on';
else
    gui_selfcalOpt = 'on';
    gui_imagepanelVis = 'off';
end
...
function create_particlepanel_controls(prana_fig,gui_selfcal)
...
% --- create exppanel with controls for defining the physical exp. params
function create_exppanel_controls(prana_fig,gui_selfcal)
...
appdata = [];
appdata.lastValidTag = 'rpcdiameter_label';
h240 = uicontrol(...
'Parent',h_gridsetuppanel,...
'FontUnits',get(0,'defaultuicontrolFontUnits'),...
'String','Particle Diameter (pix)',...
'Style','text',...
'Position',[27 21 145 16],...
'Children',[],...
'Tag','rpcdiameter_label',...
'UserData',[],...
'FontSize',10,...
'CreateFcn', {@local_CreateFcn, blanks(0), appdata} );
...
'Parent',h_gridsetuppanel,...
'FontUnits',get(0,'defaultuicontrolFontUnits'),...
'String',{  'Standard Cross Correlation (SCC)'; 'Robust Phase Correlation (RPC)'; 'Dynamic Robust Phase Correlation (DRPC)';...
            'Generalized Cross Correlation (GCC)'; 'Fraction-Filter Weighted Correlation (FWC)'; 'Spectral Phase Correlation (SPC)'; ...
            'Direct Cross Correlation (DCC)'; 'High Speed Cross Correlation (hsSCC)'; 'WFFT w/ Local Spect. Analysis (WFT-LSA)'; ...
            'RPC for Grid Images (RPCG)'},...
'Style','popupmenu',...
'Value',1,...
