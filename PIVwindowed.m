        ...
        if size(im1,3) == 3
            ...
            for n=1:length(X)
                ...
                if HSSCC
                    ...
                    G = zeros(5,1);
                    G(3) = sum(sum(region1.*region2));
                    G(1) = sum(sum(region1.*circshift(region2,+1,2)));
                    G(5) = sum(sum(region1.*circshift(region2,-1,2)));
                    G(2) = sum(sum(region1.*circshift(region2,+1,1)));
                    G(4) = sum(sum(region1.*circshift(region2,-1,1)));
